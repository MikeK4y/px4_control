...
class MavrosOdometry : public BaseSensor {
 ...
 private:
  ...
  Eigen::MatrixXd getHmat(const Eigen::MatrixXd& state) {
    ...
    double sy = sin(state(6, 1));
    double cy = cos(state(6, 1));
    double sp = sin(state(7, 1));
    double cp = cos(state(7, 1));
    double sr = sin(state(8, 1));
    double cr = cos(state(8, 1));
    ...
    H_mat(3, 4) = sy * cp;
    H_mat(3, 5) = -sp;
    H_mat(3, 6) = -sy * cp * state(3, 1) + cy * cp * state(4, 1);
    H_mat(3, 7) =
        -cy * sp * state(3, 1) - sy * sp * state(4, 1) - cp * state(5, 1);

    // h5 = (cyspsr - sycr)*xdot + (syspsr + cycr)*ydot + cpsr*zdot
    H_mat(4, 3) = cy * sp * sr - sy * cr;
    H_mat(4, 4) = sy * sp * sr + cy * cr;
    H_mat(4, 5) = cp * sr;
    H_mat(4, 6) = -(sy * sp * sr + cy * cr) * state(3, 1) +
                  (cy * sp * sr - sy * cr) * state(4, 1);
    H_mat(4, 7) = (cy * cp * sr) * state(3, 1) + (sy * cp * sr) * state(4, 1) -
                  sp * sr * state(5, 1);
    H_mat(4, 8) = (cy * sp * cr + sy * sr) * state(3, 1) +
                  (sy * sp * cr - cy * sr) * state(4, 1) +
                  cp * cr * state(5, 1);
    ...
    H_mat(5, 5) = cp * cr;
    H_mat(5, 6) = -(sy * sp * cr - cy * sr) * state(3, 1) +
                  (cy * sp * cr + sy * sr) * state(4, 1);
    H_mat(5, 7) = (cy * cp * cr) * state(3, 1) + (sy * cp * cr) * state(4, 1) -
                  sp * cr * state(5, 1);
    H_mat(5, 8) = -(cy * sp * sr - sy * cr) * state(3, 1) -
                  (sy * sp * sr + cy * cr) * state(4, 1) -
                  cp * sr * state(5, 1);
  ...
  Eigen::MatrixXd getYExpected(const Eigen::MatrixXd& state) {
    Eigen::Matrix3d Rot_mat =
        eulerToRotMat(state(6, 1), state(7, 1), state(8, 1));
